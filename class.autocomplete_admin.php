...
class AutoComplete_Admin {
	...
	public static function admin_help() {
			...
			if ( !AutoComplete::get_api_key() || ( isset( $_GET['view'] ) && $_GET['view'] == 'start' ) ) {
				...
				$current_screen->add_help_tab(
					array(
						...
						'content'	=>
							'<p><strong>' . esc_html__( 'AutoComplete.sh Setup' , 'autocomplete') . '</strong></p>' .
							'<p>' . esc_html__( "AutoComplete.sh auto-completes your blog posts, so you can focus on more important things." , 'autocomplete') . '</p>' .
				...
				$current_screen->add_help_tab(
					array(
						...
						'content'	=>
							'<p><strong>' . esc_html__( 'AutoComplete.sh Setup' , 'autocomplete') . '</strong></p>' .
							'<p>' . esc_html__( 'You need to enter an Autocomplete.sh API key to make use of this plugin on your site.' , 'autocomplete') . '</p>' .
							'<p>' . sprintf( __( 'Sign up for an account on %s to get an API Key.' , 'autocomplete'), '<a href="' . autocomplete_url() . '" target="_blank">AutoComplete.sh</a>' ) . '</p>',
					)
				);
				...
				$current_screen->add_help_tab(
					array(
						...
						'title'		=> __( 'Overview' , 'autocomplete'),
						'content'	=>
							'<p><strong>' . esc_html__( 'AutoComplete.sh Configuration' , 'autocomplete') . '</strong></p>' .
							'<p>' . esc_html__( 'autocomplete auto-completes your blog posts, so you can focus on more important things.' , 'autocomplete') . '</p>' .
							'<p>' . esc_html__( 'On this page, you are able to update your autocomplete settings and view spam stats.' , 'autocomplete') . '</p>',
					)
				);
				...
				$current_screen->add_help_tab(
					array(
						...
						'content'	=>
							...
							( AutoComplete::predefined_api_key() ? '' : '<p><strong>' . esc_html__( 'API Key' , 'autocomplete') . '</strong> - ' . esc_html__( 'Enter/remove an API key.' , 'autocomplete') . '</p>' ) .
							'<p><strong>' . esc_html__( 'Comments' , 'autocomplete') . '</strong> - ' . esc_html__( 'Show the number of approved comments beside each comment author in the comments list page.' , 'autocomplete') . '</p>' .
							'<p><strong>' . esc_html__( 'Strictness' , 'autocomplete') . '</strong> - ' . esc_html__( 'Choose to either discard the worst spam automatically or to always put all spam in spam folder.' , 'autocomplete') . '</p>',
					)
				);
			}
		}
		...
		$current_screen->set_help_sidebar(
			...
			'<p><a href="' . autocomplete_url('documentation#faq') . '" target="_blank">'     . esc_html__( 'AutoComplete.sh FAQ' , 'autocomplete') . '</a></p>' .
			'<p><a href="https://autocomplete.com/support/" target="_blank">' . esc_html__( 'AutoComplete.sh Support' , 'autocomplete') . '</a></p>'
		);
	}
